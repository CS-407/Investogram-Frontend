...
const ListPage = () => {
	...
	if (!user) {
		return <div>You must be logged in to view lists</div>;
	}

	if (
		!list.list_owner.followers_list.includes(user._id) &&
		list.list_owner._id !== user._id
	) {
		return (
			<div className="text-center">
				...
				<h2 className="text-xl">
					You must follow the list owner to view this list.
				</h2>
			</div>
		);
	}
